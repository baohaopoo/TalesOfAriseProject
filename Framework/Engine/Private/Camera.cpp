...
void CCamera::Tick(_double TimeDelta)
{
	...
	case CAMERA:
		...
		pPipeLine->Set_CamFar(m_CameraDesc.fFar);
		break;
	case LIGHT:
		ViewMatrix = m_pTransformCom->Get_WorldMatrixInverse();
		pPipeLine->Set_LightTransform(CPipeLine::D3DTS_VIEW, ViewMatrix);
		ProjMatrix = XMMatrixPerspectiveFovLH(m_CameraDesc.fFovy, m_CameraDesc.fAspect, m_CameraDesc.fNear, m_CameraDesc.fFar);
		pPipeLine->Set_LightTransform(CPipeLine::D3DTS_PROJ, ProjMatrix);
		pPipeLine->Set_LightFar(m_CameraDesc.fFar);
		break;
	}
