...
[numthreads(1024, 1, 1)]
void CS_MAIN_DOWNSCALE_FIRSTPASS(uint3 groupId : SV_GroupID, uint3 groupThreadId : SV_GroupThreadID, uint3 dispatchThreadId : SV_DispatchThreadID)
{
...
void CS_MAIN_DOWNSCALE_SECONDPASS(uint3 groupId : SV_GroupID, uint3 groupThreadId : SV_GroupThreadID, uint3 dispatchThreadId : SV_DispatchThreadID)
{
	float avgLum = 0.0;
...
[numthreads(1024, 1, 1)]
void CS_MAIN_BLOOMREVEAL(uint3 dispatchThreadId : SV_DispatchThreadID)
{
	uint2 CurPixel = uint2(dispatchThreadId.x % Res.x, dispatchThreadId.x / Res.x);
...
technique11 DefaultTechnique
{
	pass FirstPass//0
	{
		...
		GeometryShader = NULL;
		PixelShader = NULL;
		ComputeShader = compile cs_5_0 CS_MAIN_DOWNSCALE_FIRSTPASS();
	}
	...
	pass SecondPass//1
	{
		...
		GeometryShader = NULL;
		PixelShader = NULL;
		ComputeShader = compile cs_5_0 CS_MAIN_DOWNSCALE_SECONDPASS();
	}
	...
	pass BloomReveal//2
	{
		...
		GeometryShader = NULL;
		PixelShader = NULL;
		ComputeShader = compile cs_5_0 CS_MAIN_BLOOMREVEAL();
	}
}