...
HRESULT UIGlow::Render()
{
	...
	if (!m_isBattleOn) {
		...
		if (FAILED(m_pShader->Set_RawValue("glowrgb", &rgb, sizeof(float))))
			return E_FAIL;
	}
...
{
	if (FAILED(__super::SetUp_Component(L"Com_Texture", LEVEL_STATIC, L"Prototype_Component_Texture_GlowUI", (CComponent**)&m_pTexture)))
	{
